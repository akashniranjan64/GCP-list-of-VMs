...
#       print the network details of the VMs
#        print(data['networkInterfaces'])


#       print the labels of the VMs
        for key in data:
            if 'labels' in key:
                #print(instance)
                #print(data['labels'], end='\n')
                js1 = json.dumps(data['labels'])
                js = json.loads(js1)

                if 'goog-dataproc-cluster-uuid' in js:
                    print(project)
                    print(instance)
                    print(js['keep-alive'])
                    print('***********************')
